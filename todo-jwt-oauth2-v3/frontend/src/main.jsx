...
function TodoApp({ token, onLogout }) {
  ...
  async function load() {
    setTodos((await api.get("/api/todos")).data);
  }
  useEffect(() => {
    load().catch((x) => {
      if (x.response?.status === 401) onLogout();
    });
  }, []);
  async function add() {
  ...
  return (
    <div className="card wide">
      ...
      <div className="add">
        <input value={title} placeholder="Todo title" onChange={(x) => setTitle(x.target.value)} />
        <button onClick={add}>Add</button>
      </div>
      <ul>
        {todos.map((t) => (
          <li key={t.id}>
            <span className={t.completed ? "done" : ""} onClick={() => toggle(t)}>
              {t.title}
            </span>
...
function App() {
  ...
  async function logout() {
    ...
    if (t) {
      try {
        await axios.post(`${API}/api/auth/logout`, {}, { headers: { Authorization: `Bearer ${t}` } });
      } catch {}
    }
  }
