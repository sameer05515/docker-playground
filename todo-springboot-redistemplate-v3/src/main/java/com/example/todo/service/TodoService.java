...
public class TodoService {
  ...
  public TodoService(TodoRepository repo, TodoCache cache) {
    this.repo = repo;
    this.cache = cache;
  }
  ...
  public TodoResponse create(TodoRequest r) {
    Todo t = repo.save(Todo.builder().title(r.title()).description(r.description())
        .completed(Boolean.TRUE.equals(r.completed())).build());
    TodoResponse x = toResponse(t);
    cache.put(x);
    return x;
  }
  ...
  @Transactional(readOnly = true)
  public PageResponse<TodoResponse> search(String keyword, Boolean completed, int page, int size) {
  ...
  public TodoResponse update(Long id, TodoRequest r) {
    ...
    t.setCompleted(Boolean.TRUE.equals(r.completed()));
    TodoResponse x = toResponse(repo.save(t));
    cache.put(x);
    return x;
  }
