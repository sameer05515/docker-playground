...
public class KnowledgeService {
    ...
    public List<Knowledge> importKnowledge(List<Knowledge> knowledgeList) {
        Instant now = Instant.now();
        for (Knowledge knowledge : knowledgeList) {
            if (knowledge.getCreatedAt() == null) knowledge.setCreatedAt(now);
            knowledge.setUpdatedAt(now);
        }
        return repository.saveAll(knowledgeList);
    }
}
