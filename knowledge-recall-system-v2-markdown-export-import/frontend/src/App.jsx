...
function App() {
  ...
  async function exportKnowledge() {
    try {
      ...
      const exportData = {
        format: 'knowledge-recall', version: 2, exportedAt: new Date().toISOString(),
        items: response.data.map(item => { const {_id, ...rest} = {...item, _id: item.id}; delete rest.id; return rest })
      }
    ...
    } catch (error) { setMessage(error.response?.data?.error || 'Unable to export knowledge.') }
  }

  function openImportDialog() { importInputRef.current?.click() }
  ...
  async function importKnowledge(event) {
    ...
    try {
      ...
      if (!Array.isArray(importedItems)) throw new Error('Invalid import file. Expected an array or an object containing "items".')
      const response = await api.get('/knowledge')
      const existingIds = new Set(response.data.map(item => item.id))
      const toImport = []; let skipped = 0; let overwritten = 0
      for (const rawItem of importedItems) {
        if (!rawItem || typeof rawItem !== 'object') continue
        const importedId = rawItem._id ?? rawItem.id
        const item = {...rawItem}
        delete item._id
        if (importedId) item.id = importedId
        if (importedId && existingIds.has(importedId)) {
          const overwrite = window.confirm(`Knowledge with _id "${importedId}" already exists.\n\nOK = Overwrite existing record\nCancel = Skip this record`)
          if (!overwrite) { skipped++; continue }
          overwritten++
        }
        toImport.push(item)
      }
      if (toImport.length) await api.post('/knowledge/import', toImport)
      await loadAll()
      setMessage(`Import completed: ${toImport.length} imported, ${overwritten} overwritten, ${skipped} skipped.`)
    } catch (error) { setMessage(error.response?.data?.error || error.message || 'Unable to import knowledge.') }
  }
