...
@Service public class JwtService {
 ...
 private String build(String username,String provider){return Jwts.builder().subject(username).claim("provider",provider).claim("roles",List.of("ROLE_USER")).issuedAt(new Date()).expiration(new Date(System.currentTimeMillis()+expiration)).signWith(key).compact();}
